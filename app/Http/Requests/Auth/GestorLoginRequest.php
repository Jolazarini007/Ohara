...
class GestorLoginRequest extends FormRequest
{
    ...
    public function authenticate(): void
    {
        ...
        $gestor = \App\Models\Gestor::where('rm', $credentials['rm'])
            ->where('codigo_etec', $credentials['codigo_etec'])
            ->first();

        if (!$gestor || !password_verify($credentials['password'], $gestor->password)) {
        ...
        Auth::guard('gestor')->login($gestor, $remember);
